...
def train_cls(cfg: Config):
    ...
    for epoch in range(start_epoch, num_epochs):
        loss, train_acc = train_one_epoch_cls(
            classifier, optimizer_cls, criterion, data_loader_train, device=device, epoch=epoch, writers=writers
        )

        loss, acc = validate_cls(classifier, criterion, data_loader_val, device=device, epoch=epoch, writers=writers)
